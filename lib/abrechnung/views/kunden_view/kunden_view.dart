                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Expanded(
                                    child: Padding(
                                      padding: const EdgeInsets.all(8.0),
                                      child: Text(kunde.name, style: layStyleH4),
                                    ),
                                  ),
                                  InkWell(
                                    child: const Padding(
                                    ...
                                    onTap: () {
                                      // TODO: [ZEIT-10] Implementieren der Löschroutine für Kunden
                                      _logger('Starting the process of deleting a Kunde', 'onPressed');
                                    },
                                  ),
                                ],
                              ),
                                    ...
                                    child: Padding(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Text(
                                            EuroFormatter.withSymbolFromCentiCents(abwTaetigkeit.kostensatz),
                                            style: layStyleP2,
                                          ),
                                          const SizedBox(width: 8),
                                          InkWell(
                                            onTap: () async {
                                              ...
                                              final taetigkeit = await showDialog<Taetigkeit>(
                                                context: context,
                                                builder: (context) =>
                                                    _AddAbwTaetigkeitDialog(abwTaetigkeit: abwTaetigkeit),
                                              );
                                              _logger('Receiving from dialog: $taetigkeit', 'build-onPressed');
                                              if (taetigkeit != null) {
                                                _logger(
                                                    'Triggering AddAbwTaetigkeit for: $taetigkeit', 'build-onPressed');
                                                context.read<AbrechnungBloc>().add(AddAbwTaetigkeit(kunde, taetigkeit));
                                              }
                                            },
                                            child: const Padding(
                                              padding: EdgeInsets.all(8.0),
                              ...
                              InkWell(
                                ...
                                onTap: () async {
                                  _logger('Start process of creating an abweichenden Kostensatz, build-onTap');
                                  final taetigkeit = await showDialog<Taetigkeit>(
                                    context: context,
                                    builder: (context) => const _AddAbwTaetigkeitDialog(),
                                  );
                                  _logger('Receiving from dialog: $taetigkeit', 'build-onPressed');
                                  if (taetigkeit != null) {
                                    _logger('Triggering AddAbwTaetigkeit for: $taetigkeit', 'build-onPressed');
                                    context.read<AbrechnungBloc>().add(AddAbwTaetigkeit(kunde, taetigkeit));
                                  }
                                },
                              ),
...
class _AddAbwTaetigkeitDialog extends StatefulWidget {
  const _AddAbwTaetigkeitDialog({this.abwTaetigkeit, Key? key}) : super(key: key);
  final Taetigkeit? abwTaetigkeit;
  @override
  State<_AddAbwTaetigkeitDialog> createState() => _AddAbwTaetigkeitDialogState();
}
...
class _AddAbwTaetigkeitDialogState extends State<_AddAbwTaetigkeitDialog> {
  ...
  void initState() {
    ...
    if (widget.abwTaetigkeit != null) {
      ...
      _kostensatzController.text = EuroFormatter.withSymbolFromCentiCents(widget.abwTaetigkeit!.kostensatz);
      _kosensatzFormattedController.text = EuroFormatter.withSymbolFromCentiCents(widget.abwTaetigkeit!.kostensatz);
    }
  }
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          onChanged: (v) {
                            setState(() {
                              _kosensatzFormattedController.text = EuroFormatter.withoutSymbolFromDoubleString(v);
                            });
                          },
                        ),
                      ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton(
                          child: const Text('Ok'),
                          onPressed: () {
                            _logger('Adding a new abweichende Taetigkeit', 'build-onPressed');
                            if (_name == null ||
                                _name!.isEmpty ||
                                !state.abrechnungSettings.taetigkeiten.any((t) => t.name == _name)) {
                              _logger('No valid name given, aborting', 'build-onPressed');
                            ...
                            Navigator.of(context).pop(Taetigkeit(name: _name!, kostensatz: input));
                          },
                        ),
                      ),
                    ],
                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          ...
                          Navigator.of(context).pop(Kunde(name: nameController.text));
                        },
                      ),
                    ),
                  ],
