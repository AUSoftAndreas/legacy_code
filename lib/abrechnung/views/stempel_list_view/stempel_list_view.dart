  ...
  Widget build(BuildContext context) => BlocBuilder<AbrechnungBloc, AbrechnungState>(
        builder: (context, abrechnungstate) {
          ...
          for (var i = 0; i < stempels.length - 1; i++) {
            _logger('Comparing ${stempels[i].timestampEdited} and ${stempels[i + 1].timestampEdited}', 'build');
            _logger('-> Result: ${stempels[i + 1].timestampEdited.difference(stempels[i].timestampEdited).inMinutes}',
                'build');
            lengths.add(stempels[i + 1].timestampEdited.difference(stempels[i].timestampEdited).inMinutes);
          }
              ...
              : SingleChildScrollView(
                  child: Container(
                    ...
                    child: DataTable(
                      columns: [
                        const DataColumn(
                          label: Text('von', style: TextStyle(fontWeight: FontWeight.bold)),
                        ),
                        const DataColumn(
                          label: Text('bis', style: TextStyle(fontWeight: FontWeight.bold)),
                        ),
                        const DataColumn(
                          label: Text('Dauer(m)', style: TextStyle(fontWeight: FontWeight.bold)),
                        ),
                        const DataColumn(
                          label: Text('Typ', style: TextStyle(fontWeight: FontWeight.bold)),
                        ),
                      ],
                      rows: [
                        for (var i = 0; i < stempels.length - 1; i++)
                          DataRow(
                            cells: [
                              DataCell(
                                Text(
                                  DateFormatter.HHmm(stempels[i].timestampEdited),
                                ),
                              ),
                              DataCell(
                                Text(
                                  DateFormatter.HHmm(stempels[i + 1].timestampEdited),
                                ),
                              ),
                              DataCell(
                                Text(
                        ...
                        if (stempels.isNotEmpty && stempels.last is! EndeStempel)
                          DataRow(
                            cells: [
                              DataCell(
                                Text(
                                  DateFormatter.HHmm(stempels.last.timestampEdited),
                                ),
                              ),
                              const DataCell(
                                Text(
