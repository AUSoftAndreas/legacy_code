...
class _ActionButton extends StatelessWidget {
  const _ActionButton({required this.child, required this.onPressed, Key? key});
  final Widget child;
...
class ActionButtonStarteArbeitszeit extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => BlocBuilder<AbrechnungBloc, AbrechnungState>(
  ...
  Widget build(BuildContext context) => BlocBuilder<AbrechnungBloc, AbrechnungState>(
        builder: (context, abrechnungState) => _ActionButton(
          ...
          onPressed: () async {
            _logger('Button pressed', 'build-onPressed');
            Einsatz? einsatz;
            final taetigkeiten = abrechnungState.abrechnungSettings.taetigkeiten;
            ...
            if (einsatz == null) {
              context.read<AbrechnungBloc>().add(SignalProblem(EinsatzNotDefined()));
              return;
            }
            ;
            context.read<AbrechnungBloc>().add(
...
class ActionButtonStarteFahrt extends StatelessWidget {
  ...
  Widget build(BuildContext context) => BlocBuilder<AbrechnungBloc, AbrechnungState>(
        builder: (context, abrechnungState) => _ActionButton(
          ...
          onPressed: () async {
            _logger('Button pressed', 'build-onPressed');
            final currentStempel = abrechnungState.abrechnungPeriode.currentSchicht.currentStempel;
            if (currentStempel != null && currentStempel is EinsatzStempel) {
              ...
              _logger('shouldContinueEinsatz: $shouldContinueEinsatz', 'build-onPressed');
              if (shouldContinueEinsatz != null && shouldContinueEinsatz) {
                _logger('Continue Einsatz', 'build-onPressed');
                _logger('Current Stempel Einsatz: ${currentStempel.einsatz}', 'build-onPressed');
                context.read<AbrechnungBloc>().add(
                      StarteFahrt(
                        timestamp: DateTime.now(),
                        timestampEdited: DateTime.now(),
  ...
  Widget build(BuildContext context) => BlocBuilder<AbrechnungBloc, AbrechnungState>(
        builder: (context, abrechnungState) => _ActionButton(
          ...
          onPressed: () async {
            ...
            final stempelBeforePause = abrechnungState.abrechnungPeriode.currentSchicht
                .stempels[abrechnungState.abrechnungPeriode.currentSchicht.stempels.length - 2];
            Einsatz? einsatz;
...
class _EinsatzDialog extends StatefulWidget {
  const _EinsatzDialog({required this.taetigkeiten, required this.kunden, Key? key}) : super(key: key);
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // Dialog-Element
                  // TODO: [ZEIT-46] ActionButtons/_EinsatzDialog: Dropdown-Felder Größe anpassen
                        ...
                        child: ElevatedButton(
                          child: Row(
                            children: [
                              ...
                              const Expanded(child: Center(child: Text('Nein'))),
                            ],
                          ),
                          onPressed: () {
                        ...
                        child: ElevatedButton(
                          child: Row(
                            children: [
                              ...
                              const Expanded(child: Center(child: Text('Ja'))),
                            ],
                          ),
                          onPressed: () {
                            if (kunde != null && taetigkeit != null) {
                              Navigator.of(context)
                                  .pop(Einsatz(kundeName: kunde!.name, taetigkeitName: taetigkeit!.name));
                            }
                          },
                        ),
                      ),
...
class _FahrtDialog extends StatefulWidget {
  const _FahrtDialog({required this.taetigkeiten, required this.kunden, Key? key}) : super(key: key);
  final List<Taetigkeit> taetigkeiten;
  final List<Kunde> kunden;
