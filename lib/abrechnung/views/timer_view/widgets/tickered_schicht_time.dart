...
class _TickeredSchichtTimeState extends State<TickeredSchichtTime> {
  ...
  Widget build(BuildContext context) => BlocBuilder<AbrechnungBloc, AbrechnungState>(
        builder: (context, abrechnungState) {
          ...
          final schichtBeginn = schicht.stempels.isNotEmpty ? schicht.stempels.first.timestampEdited : null;
          final schichtEnde = schicht.stempels.isEmpty
              ? null
              : (schicht.stempels.last is! EndeStempel ? _time : schicht.stempels.last.timestampEdited);
          final duration =
              (schichtBeginn != null && schichtEnde != null) ? schichtEnde.difference(schichtBeginn) : null;
          final durationFormatted = duration != null ? DurationFormatter.HHmm(duration) : '--:--';
          final isActive = schicht.stempels.isNotEmpty && schicht.stempels.last is! EndeStempel;
