...
class AbrechnungBloc extends Bloc<AbrechnungEvent, AbrechnungState> {
  ...
  void _addSchicht(Emitter<AbrechnungState> emit) {
    ...
    final newState = ap.copyWith(schichten: [...ap.schichten, const Schicht(stempels: [])]);
    emit(AbrechnungState(abrechnungPeriode: newState, abrechnungSettings: state.abrechnungSettings));
  }
  ...
  void _addStempel(Stempel stempel, Emitter<AbrechnungState> emit) {
    ...
    final newSchichten = ap.schichten.map((s) => s == currentSchicht ? newSchicht : s).toList();
    final newAp = ap.copyWith(schichten: newSchichten);
    emit(AbrechnungState(abrechnungPeriode: newAp, abrechnungSettings: state.abrechnungSettings));
  }
  ...
  void _addTaetigkeit(Taetigkeit taetigkeit, Emitter<AbrechnungState> emit) {
    ...
    final containsTaetigkeit = oldTaetigkeiten.any((t) => t.name == taetigkeit.name);
    if (containsTaetigkeit) {
      _logger('Error because Taetigkeit already exists');
      _throw(TaetigkeitAlreadyExistingProblem(), emit);
      return;
    }
    final newTaetigkeiten = [...oldTaetigkeiten, taetigkeit]..sort((a, b) => a.name.compareTo(b.name));
    final newASettings = state.abrechnungSettings.copyWith(taetigkeiten: newTaetigkeiten);
    _logger('Emitting new state: $newASettings');
    emit(AbrechnungState(abrechnungPeriode: state.abrechnungPeriode, abrechnungSettings: newASettings));
  }

  void _addAbwTaetigkeit(Kunde kunde, Taetigkeit taetigkeit, Emitter<AbrechnungState> emit) {
    ...
    if (!state.abrechnungSettings.taetigkeiten.any((t) => t.name == taetigkeit.name)) {
    ...
    if (kunde.abwTaetigeiten.any((abw) => abw.name == taetigkeit.name)) {
      ...
      newAbwTaetigkeiten = kunde.abwTaetigeiten.map((abw) => abw.name == taetigkeit.name ? taetigkeit : abw).toList();
    } else {
      _logger('AbwTaetigkeit does not already exist -> We add this abwTaetigkeit and sort the list');
      newAbwTaetigkeiten = [...kunde.abwTaetigeiten, taetigkeit]..sort((a, b) => a.name.compareTo(b.name));
    }
    final newKunde = kunde.copyWith(abwTaetigeiten: newAbwTaetigkeiten);
    final newKunden = state.abrechnungSettings.kunden.map((k) => k == kunde ? newKunde : k).toList();
    final newAbrechnungSettings = state.abrechnungSettings.copyWith(kunden: newKunden);
    emit(AbrechnungState(abrechnungPeriode: state.abrechnungPeriode, abrechnungSettings: newAbrechnungSettings));
  }
  ...
  void _addKunde(Kunde kunde, Emitter<AbrechnungState> emit) {
    ...
    final newKunden = [...oldKunden, kunde]..sort((a, b) => a.name.compareTo(b.name));
  ...
  void _beendePause({
    ...
    lastStempel.maybeWhen(
      ...
      pause: (_, __) {
        if (schicht.stempels.any((s) => s is EinsatzStempel) && continueEinsatz) {
          final lastEinsatz = schicht.stempels.lastWhere((s) => s is EinsatzStempel) as EinsatzStempel;
          _addStempel(
            EinsatzStempel(
              timestamp: timestamp ?? DateTime.now(),
              timestampEdited: timestampEdited,
  ...
  void _removeKunde(Kunde kunde, Emitter<AbrechnungState> emit) {
    ...
    if (state.abrechnungPeriode.schichten.any((schicht) => schicht.stempels.any((stempel) => stempel.maybeWhen(
        einsatz: (_, __, e) => e.kundeName == kunde.name,
        fahrt: (_, __, e) => e?.kundeName == kunde.name,
        orElse: () => false)))) {
      _logger('Error because Kunde is already used in this Abrechnung', '_removeTaetigkeit');
  ...
  void _removeTaetigkeit(Taetigkeit taetigkeit, Emitter<AbrechnungState> emit) {
    ...
    if (state.abrechnungPeriode.schichten.any((schicht) => schicht.stempels.any((stempel) => stempel.maybeWhen(
        ...
        orElse: () => false)))) {
      _logger('Error because Taetigkeit is already used in this Abrechnung', '_removeTaetigkeit');
      _throw(TaetigkeitAlreadyUsedProblem(), emit);
      return;
    }
    final newTaetigkeiten = oldTaetigkeiten.where((t) => t != taetigkeit).toList();
    final newASettings = state.abrechnungSettings.copyWith(taetigkeiten: newTaetigkeiten);
    _logger('Emitting new state: $newASettings');
    emit(AbrechnungState(abrechnungPeriode: state.abrechnungPeriode, abrechnungSettings: newASettings));
  }

  void _removeAbwTaetigkeit(Kunde kunde, Taetigkeit taetigkeit, Emitter<AbrechnungState> emit) {
    ...
    final newAbwTaetigkeiten = kunde.abwTaetigeiten.where((abw) => abw != taetigkeit).toList();
    final newKunde = kunde.copyWith(abwTaetigeiten: newAbwTaetigkeiten);
    final newKunden = state.abrechnungSettings.kunden.map((k) => k == kunde ? newKunde : k).toList();
    final newAbrechnungSettings = state.abrechnungSettings.copyWith(kunden: newKunden);
    emit(AbrechnungState(abrechnungPeriode: state.abrechnungPeriode, abrechnungSettings: newAbrechnungSettings));
