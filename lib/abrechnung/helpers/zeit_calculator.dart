...
class ZeitCalculator {
  ...
  /// TODO: [ZEIT-38] Replace the map solution with a dedicated object holding the data
  Map<String, int> getMinutesForSchicht() {
    ...
    for (var i = 0; i < schicht!.stempels.length - 1; i++) {
      ...
      final start = _roundDateTimeAccordingToSettings(currStempel.timestampEdited);
      final ende = _roundDateTimeAccordingToSettings(nextStempel.timestampEdited);
      final diff = ende.difference(start).inMinutes;
      currStempel.maybeWhen(
        arbeitszeit: (ts, tse) {
          map[currStempel.typeString] = (map[currStempel.typeString] ?? 0) + diff;
        },
        pause: (ts, tse) {
          map[currStempel.typeString] = (map[currStempel.typeString] ?? 0) + diff;
        },
        fahrt: (ts, tse, taetigkeit) {
          if (taetigkeit != null) {
            map['Einsatz@$taetigkeit'] = (map['Einsatz@$taetigkeit'] ?? 0) + diff;
          } else {
            map['${currStempel.typeString}'] = (map['${currStempel.typeString}'] ?? 0) + diff;
          }
        },
        einsatz: (ts, tse, einsatz) {
          map['${currStempel.typeString}@${einsatz.taetigkeitName}@${einsatz.kundeName}'] =
              (map['${currStempel.typeString}@${einsatz.taetigkeitName}@${einsatz.kundeName}'] ?? 0) + diff;
        },
        orElse: () {},
      );
    }
  ...
  DateTime _roundDateTimeAccordingToSettings(DateTime dateTime) {
    final inMinutes = dateTime.difference(DateTime(dateTime.year, dateTime.month, dateTime.day)).inMinutes;
    final inMinutesRounded = inMinutes % abrechnungSettings.rundenMinuten == 0
        ? (inMinutes ~/ abrechnungSettings.rundenMinuten) * abrechnungSettings.rundenMinuten
        : (inMinutes ~/ abrechnungSettings.rundenMinuten + 1) * abrechnungSettings.rundenMinuten;
    final rounded = DateTime(dateTime.year, dateTime.month, dateTime.day).add(Duration(minutes: inMinutesRounded));
    return rounded;
  }
}
